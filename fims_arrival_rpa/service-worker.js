...
async function processArrivalStudent({ tabId, student, config }) {
  ...
  const filled = await sendAction(tabId, editFrame.frameId, 'FILL_ARRIVAL_EDIT', {
    ...
    // 엑셀에 값이 있는 경우에만 덮어쓴다. 비어 있으면 FIMS 값을 그대로 둔다.
  ...
  const schoolNote = filled.data?.schoolSanitized
    ...
    : '';
  const optionalNote = [
    filled.data?.optionalApplied?.length ? ` / 추가입력: ${filled.data.optionalApplied.join(', ')}` : '',
    filled.data?.optionalFailed?.length ? ` / 추가입력 실패: ${filled.data.optionalFailed.join(', ')}` : ''
  ].join('');
  ...
  try {
    ...
    if (arrivalDate && verification.data?.arrivalConfirmed === true) {
      return {
        ...
        arrivalDate,
        note: '',
        detail: `학번·입국·입학일자 저장 완료 / ${arrivalDate} (입국) 확인${verification.data?.arrivalMarked === true ? '' : ' (수정화면 입국일자 기준)'}${schoolNote}${optionalNote}${dialogMessages.length ? ` / 알림: ${dialogMessages.at(-1)}` : ''}`
      };
    }
    ...
    return {
      ...
      detail: `저장 완료 알림은 확인했으나 상세화면에 입국일자가 표시되지 않았습니다(관서 출입국 기록 미연결로 보임).${schoolNote}${optionalNote}${dialogMessages.length ? ` 알림: ${dialogMessages.at(-1)}` : ''}`
    };
  } catch (error) {
    return {
      code: 'SAVE_FAILED',
